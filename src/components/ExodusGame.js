...
const ExodusGame = () => {
  ...
  return (
    <div className="h-screen bg-gray-900 text-white p-4 flex flex-col overflow-auto bg-[url('/space-background.jpg')] bg-cover bg-center">
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-3 gap-6 mb-6">
          <div className="bg-gray-800 bg-opacity-75 rounded-lg p-6 shadow-lg border border-blue-500 backdrop-filter backdrop-blur-sm">
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              {Object.entries(resources).map(([resource, amount]) => (
                <div key={resource} className="bg-gray-700 rounded-lg p-3 flex items-center justify-between">
          ...
          <div className="bg-gray-800 bg-opacity-75 rounded-lg p-6 shadow-lg border border-blue-500 backdrop-filter backdrop-blur-sm col-span-2">
              ...
              {grid.map((row, rowIndex) => (
                row.map((cell, colIndex) => (
                  <div
                    ...
                    className="w-12 h-12 border border-gray-600 flex items-center justify-center cursor-pointer hover:bg-blue-700 text-3xl transition-all duration-200 transform hover:scale-110"
            ...
            <div className="grid grid-cols-2 gap-3">
              {Object.entries(TECHS).map(([tech, { name, effect, cost }]) => (
                ...
                >
                  ...
                  <div className="text-xs">{cost} RP</div>
                  {showTechInfo && <p className="text-xs mt-1">{effect}</p>}
                </button>
              ))}
            </div>
            <button 
          ...
          <div className="bg-gray-800 bg-opacity-75 rounded-lg p-6 shadow-lg border border-blue-500 backdrop-filter backdrop-blur-sm">
            ...
            {disasters.length > 0 ? (
              disasters.map((disaster, index) => (
                <div key={index} className="bg-red-900 p-3 rounded mb-3">
        ...
        <div className="mt-6 bg-gray-800 bg-opacity-75 p-4 rounded-lg shadow-lg border border-blue-500 backdrop-filter backdrop-blur-sm flex justify-between items-center">
          <div>
          ...
          <div className="flex items-center">
            <button 
              className="px-4 py-2 bg-yellow-500 hover:bg-yellow-400 rounded mr-4 text-black font-bold"
              onClick={() => setShowTutorial(true)}
            >
              Show Tutorial
            </button>
            <button 
              className="px-6 py-3 bg-blue-500 hover:bg-blue-400 rounded text-lg font-bold transition-all duration-200 transform hover:scale-105"
              onClick={nextTurn}
            >
              Next Turn
            </button>
